...
export const PI_SIGNIN_ONCLICK =
  ...
  ");})]).then(function(){},function(){if(!st.done&&!window.__PI_AUTH_OK)resetHang();});}else{armHangTimer(st);}}catch(tm){armHangTimer(st);}}else if(!p){setBtnBusy(false);}}catch(w2){setBtnBusy(false);console.log('[Pi] error: '+errMsg(w2));showMsg(failText(w2));}}" +
    "function sandboxFlag(){try{if(typeof window.__YOUNEON_PI_SANDBOX__==='boolean')return window.__YOUNEON_PI_SANDBOX__;var h=String((location&&location.hostname)||'');try{window.__YOUNEON_PI_HOST__=h;}catch(hh){}}catch(e){}return true;}" +
...
export const PI_SIGNIN_CONTROL_STYLE =
  "height:52px;min-height:52px;padding:0 22px;font-size:16px;font-weight:800;line-height:1;border:0;border-radius:999px;color:#FFFFFF;background-color:#C21875;background-image:none;cursor:pointer;width:100%;max-width:100%;display:flex;align-items:center;justify-content:center;gap:8px;box-sizing:border-box;font-family:system-ui,-apple-system,Segoe UI,sans-serif;pointer-events:auto !important;position:relative !important;z-index:2147483647 !important;touch-action:manipulation;-webkit-tap-highlight-color:rgba(194,24,117,0.35);user-select:none;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;-webkit-touch-callout:none;caret-color:transparent;-webkit-appearance:none;appearance:none;box-shadow:0 0 22px rgba(194,24,117,0.55);letter-spacing:0.01em";
